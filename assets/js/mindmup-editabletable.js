...
$.fn.editableTableWidget = function (options) {
	...
	return $(this).each(function () {
			...
			showEditor = function (select) {
				...
				if (active.length && !active.hasClass(activeOptions.disableClass)) {
					editor.val(active.text())
						.removeClass('autocomplete-textarea')
						.addClass(active.attr("class")+'-textarea')
						.removeClass('error')
						.attr('position',active.parent().index())
						.show()
		...
		editor.blur(function () {
			...
			if (e.which === ENTER) {
				// Change text when user clicks enter button..
				var eq = $(".autocomplete-textarea").attr("position");  
				var product_selected = $(".focus").text();  
				if(product_selected != "No Result Found"){
					var hsn = $(".focus").attr("hsn");  
					var mrp = $(".focus").attr("rate");  
					var gst = $(".focus").attr("gst");  
				...
				e.preventDefault();
				e.stopPropagation();
				if(product_selected != "" && product_selected != "No Result Found"){
					$(".autocomplete:eq('"+eq+"')").text(product_selected);
				}
			} else if (e.which === ESC) {
				editor.val(active.text());
